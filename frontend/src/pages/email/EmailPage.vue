...
const activeTab = ref<'accounts' | 'forwarders' | 'autoresponders' | 'webmail'>(
    'accounts',
);
...
const stats = computed(() => ({
    ...
    activeAutoresponders: autoresponders.value.filter((a) => a.is_active)
        .length,
}));
...
const createForwarder = async (): Promise<void> => {
    if (
        !forwarderSource.value.trim() ||
        !forwarderDest.value.trim() ||
        !fwdDomainId.value
    ) {
        error.value = 'Semua field harus diisi';
        return;
...
const deleteForwarder = async (): Promise<void> => {
    ...
    try {
        ...
        error.value =
            err.response?.data?.message || 'Gagal menghapus forwarder';
    } finally {
        isLoading.value = false;
    }
};
...
const createAutoresponder = async (): Promise<void> => {
    ...
    try {
        ...
        error.value =
            err.response?.data?.message || 'Gagal membuat autoresponder';
    } finally {
        isLoading.value = false;
    }
};
...
const deleteAutoresponder = async (): Promise<void> => {
    ...
    try {
        ...
        error.value =
            err.response?.data?.message || 'Gagal menghapus autoresponder';
    } finally {
        isLoading.value = false;
    }
};
    ...
    <MainLayout>
        <div class="max-w-7xl mx-auto space-y-6 animate-in">
            ...
            >
                <div>
                    ...
                    <p class="text-muted-foreground mt-1 text-sm">
                        Manage email accounts, forwarders, and autoresponders.
                    </p>
                </div>
            ...
            <div class="grid grid-cols-2 md:grid-cols-5 gap-4">
                <div
                    v-for="stat in [
                        {
                            label: 'Accounts',
                            val: stats.totalAccounts,
                            icon: User,
                            color: 'text-primary',
                        },
                        {
                            label: 'Used',
                            val: formatMb(stats.usedQuota),
                            icon: HardDrive,
                            color: 'text-blue-500',
                        },
                        {
                            label: 'Quota',
                            val: formatMb(stats.totalQuota),
                            icon: HardDrive,
                            color: 'text-emerald-500',
                        },
                        {
                            label: 'Forwarders',
                            val: stats.activeForwarders,
                            icon: Forward,
                            color: 'text-indigo-500',
                        },
                        {
                            label: 'Autoresponders',
                            val: stats.activeAutoresponders,
                            icon: RefreshCcw,
            ...
            >
                ...
                <!-- Error -->
                <div
                    v-else-if="error && !accounts.length"
                            ...
                            >
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-3">
                                        <div
                                            class="w-10 h-10 bg-primary/10 rounded-xl flex items-center justify-center text-primary group-hover:bg-primary group-hover:text-primary-foreground transition-all"
                                        >
                                            <Mail class="w-5 h-5" />
                                        </div>
                                        <div>
                                ...
                                <td class="px-6 py-4">
                                    <span
                                        ...
                                        >{{ formatMb(account.quota_mb) }}</span
                                    >
                                </td>
                                <td class="px-6 py-4">
                                    <div class="w-24">
                                ...
                                <td class="px-6 py-4">
                                    <div
                                        :class="[
                                            'inline-flex items-center gap-1 px-2 py-1 rounded-lg text-[10px] font-semibold uppercase',
                                            account.is_active
                                ...
                                <td class="px-6 py-4 text-right">
                                    ...
                                    >
                                        ...
                                        <button
                                            @click="openChangePassword(account)"
                                        ...
                                        <button
                                            @click="openDeleteAccount(account)"
                        ...
                        <tbody class="divide-y divide-border">
                            ...
                            >
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-3">
                                        <div
                                            class="w-10 h-10 bg-indigo-500/10 rounded-xl flex items-center justify-center text-indigo-600"
                                        >
                                            <Send class="w-5 h-5" />
                                        </div>
                                        <p
                ...
                <div v-else-if="activeTab === 'autoresponders'">
                    ...
                    >
                        ...
                        <p class="text-muted-foreground mt-2 text-sm">
                            Set up vacation or out-of-office replies.
                        </p>
                    </div>
                    <div
                        v-else
                    ...
                    >
                        ...
                        >
                            <div class="flex items-start justify-between mb-4">
                                ...
                                <button
                                    @click="openDeleteAutoresponder(ar)"
                                    class="p-2 text-muted-foreground hover:text-destructive hover:bg-destructive/10 rounded-lg transition-all opacity-0 group-hover:opacity-100"
                                >
                                    <Trash2 class="w-4 h-4" />
                                </button>
                            ...
                            <div class="space-y-2">
                                ...
                                <div
                                    v-if="ar.start_date || ar.end_date"
                                    class="flex items-center gap-2 text-xs text-muted-foreground"
                                >
                                    <Calendar class="w-3.5 h-3.5" />
                                    {{
                                        ar.start_date?.split('T')[0] || 'Now'
                                    }}
                                    →
                                    {{ ar.end_date?.split('T')[0] || 'No end' }}
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div v-else>
                    ...
                    <div class="p-6">
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <a
                                href="https://roundcube.example.com"
                                target="_blank"
                                class="bg-muted/50 border border-border p-6 rounded-xl hover:border-primary/30 transition-all group"
                            >
                                <div
                                    class="w-12 h-12 bg-blue-500/10 rounded-xl flex items-center justify-center text-blue-600 mb-4 group-hover:bg-blue-500 group-hover:text-white transition-all"
                                >
                                    <Mail class="w-6 h-6" />
                                </div>
                                <h3
                            ...
                            <a
                                href="https://rainloop.example.com"
                                target="_blank"
                                class="bg-muted/50 border border-border p-6 rounded-xl hover:border-primary/30 transition-all group"
                            >
                                <div
                                    class="w-12 h-12 bg-emerald-500/10 rounded-xl flex items-center justify-center text-emerald-600 mb-4 group-hover:bg-emerald-500 group-hover:text-white transition-all"
                                >
                                    <Mail class="w-6 h-6" />
                                </div>
                                <h3
                            ...
                            <a
                                href="https://squirrelmail.example.com"
                                target="_blank"
                                class="bg-muted/50 border border-border p-6 rounded-xl hover:border-primary/30 transition-all group"
                            >
                                <div
                                    class="w-12 h-12 bg-amber-500/10 rounded-xl flex items-center justify-center text-amber-600 mb-4 group-hover:bg-amber-500 group-hover:text-white transition-all"
                                >
                                    <Mail class="w-6 h-6" />
                                </div>
                                <h3
                    ...
                    <div>
                        ...
                        <div class="flex items-center gap-2">
                            <input
                                ...
                                type="text"
                                placeholder="user"
                                class="flex-1 bg-muted border border-border rounded-lg px-4 py-3 text-sm focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none"
                            />
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <input
                                ...
                                placeholder="••••••••"
                                class="w-full bg-muted border border-border rounded-lg px-4 py-3 pr-12 text-sm focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none"
                            />
                            <button
        ...
        <div
            ...
            class="fixed inset-0 bg-black/50 flex items-center justify-center z-50"
            @click.self="showEditAccountModal = false"
        >
            <div
                class="bg-card border border-border rounded-xl shadow-xl w-full max-w-md p-6 animate-in"
            >
        ...
        <!-- Add Forwarder Modal -->
        <div
            v-if="showAddForwarderModal"
            class="fixed inset-0 bg-black/50 flex items-center justify-center z-50"
            @click.self="showAddForwarderModal = false"
        >
            <div
                class="bg-card border border-border rounded-xl shadow-xl w-full max-w-md p-6 animate-in"
            >
                <div class="flex items-center justify-between mb-6">
                    ...
                    <button
                        @click="showAddForwarderModal = false"
                        class="p-1 rounded hover:bg-muted transition-colors"
                    >
                        <X class="w-5 h-5 text-muted-foreground" />
                    </button>
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <span class="text-sm text-muted-foreground"
                                >@{{
                                    domains.find((d) => d.id === fwdDomainId)
                                        ?.domain_name || 'domain'
                                }}</span
                            >
                        </div>
        ...
        <div
            v-if="showAddAutoresponderModal"
            class="fixed inset-0 bg-black/50 flex items-center justify-center z-50"
            @click.self="showAddAutoresponderModal = false"
        >
            <div
            ...
            >
                <div class="flex items-center justify-between mb-6">
                    ...
                    <button
                        @click="showAddAutoresponderModal = false"
                        class="p-1 rounded hover:bg-muted transition-colors"
                    >
                        <X class="w-5 h-5 text-muted-foreground" />
                    </button>
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <textarea
                            ...
                            class="w-full bg-muted border border-border rounded-lg px-4 py-3 text-sm focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none resize-none"
                        />
                    </div>
                ...
                <div class="flex justify-end gap-3 mt-6">
                    <button
                        @click="
                            showAddAutoresponderModal = false;
                            error = null;
                        "
                        class="px-4 py-2 text-sm font-medium text-muted-foreground hover:text-foreground transition-colors"
                    >
        ...
        <div
            v-if="showDeleteForwarderModal"
            class="fixed inset-0 bg-black/50 flex items-center justify-center z-50"
            @click.self="showDeleteForwarderModal = false"
        >
            <div
                class="bg-card border border-border rounded-xl shadow-xl w-full max-w-md p-6 animate-in"
            >
                ...
                <p class="text-sm text-muted-foreground mb-6">
                    Delete forwarder from
                    <span class="font-semibold text-foreground">{{
                        selectedForwarder?.source_email
                    }}</span
                    >?
                </p>
                <div class="flex justify-end gap-3">
                    <button
                        @click="showDeleteForwarderModal = false"
                        class="px-4 py-2 text-sm font-medium text-muted-foreground hover:text-foreground transition-colors"
                    >
                        Cancel
                    </button>
                    <button
                        @click="deleteForwarder"
                        class="px-4 py-2 text-sm font-medium bg-destructive text-destructive-foreground rounded-lg hover:bg-destructive/90 transition-colors"
                    >
                        Delete
                    </button>
        ...
        <div
            v-if="showDeleteAutoresponderModal"
            class="fixed inset-0 bg-black/50 flex items-center justify-center z-50"
            @click.self="showDeleteAutoresponderModal = false"
        >
            <div
                class="bg-card border border-border rounded-xl shadow-xl w-full max-w-md p-6 animate-in"
            >
                ...
                <p class="text-sm text-muted-foreground mb-6">
                    Delete autoresponder for
                    <span class="font-semibold text-foreground">{{
                        selectedAutoresponder?.email_address
                    }}</span
                    >?
                </p>
                <div class="flex justify-end gap-3">
                    <button
                        @click="showDeleteAutoresponderModal = false"
                        class="px-4 py-2 text-sm font-medium text-muted-foreground hover:text-foreground transition-colors"
                    >
                        Cancel
                    </button>
