...
const activeTab = ref<
    'domains' | 'subdomains' | 'ssl' | 'dns' | 'redirects' | 'aliases'
>('domains');
...
const dnsTypes = [
    ...
    { value: 'NS', label: 'NS (Name Server)', placeholder: 'ns1.example.com' },
    {
        value: 'SRV',
        label: 'SRV (Service)',
        placeholder: '0 5 5269 xmpp.example.com',
    },
    {
        value: 'CAA',
        label: 'CAA (Cert Authority)',
...
const fetchSubFeatures = async (): Promise<void> => {
    ...
    try {
        if (activeTab.value === 'subdomains') {
            const res = await domainService.listSubdomains(
                selectedDomainId.value,
            );
            subdomains.value = res.data.data || [];
        } else if (activeTab.value === 'dns') {
            const res = await domainService.listDnsRecords(
                selectedDomainId.value,
            );
            dnsRecords.value = res.data.data || [];
        } else if (activeTab.value === 'redirects') {
            const res = await domainService.listRedirects(
                selectedDomainId.value,
            );
...
const deleteSubdomain = async (): Promise<void> => {
    ...
    try {
        isLoading.value = true;
        await domainService.deleteSubdomain(
            selectedDomainId.value,
            selectedSubdomain.value.id,
        );
        ...
        error.value =
            err.response?.data?.message || 'Gagal menghapus subdomain';
    } finally {
        isLoading.value = false;
    }
};
...
const createDnsRecord = async (): Promise<void> => {
    if (
        !newDnsName.value.trim() ||
        !newDnsValue.value.trim() ||
        !selectedDomainId.value
    )
        return;
    try {
        ...
        await domainService.createDnsRecord(selectedDomainId.value, {
            ...
            ttl: newDnsTtl.value,
            priority:
                newDnsType.value === 'MX' ? newDnsPriority.value : undefined,
        });
...
const updateDnsRecord = async (): Promise<void> => {
    ...
    try {
        ...
        await domainService.updateDnsRecord(
            selectedDomainId.value,
            editingDnsRecord.value.id,
            {
                value: newDnsValue.value.trim(),
                ttl: newDnsTtl.value,
                priority:
                    newDnsType.value === 'MX'
                        ? newDnsPriority.value
                        : undefined,
            },
        );
...
const deleteDnsRecord = async (): Promise<void> => {
    ...
    try {
        isLoading.value = true;
        await domainService.deleteDnsRecord(
            selectedDomainId.value,
            selectedDnsRecord.value.id,
        );
        ...
        error.value =
            err.response?.data?.message || 'Gagal menghapus DNS record';
    } finally {
        isLoading.value = false;
    }
};
...
const deleteRedirect = async (): Promise<void> => {
    ...
    try {
        isLoading.value = true;
        await domainService.deleteRedirect(
            selectedDomainId.value,
            selectedRedirect.value.id,
        );
...
const deleteAlias = async (): Promise<void> => {
    ...
    try {
        isLoading.value = true;
        await domainService.deleteAlias(
            selectedDomainId.value,
            selectedAlias.value.id,
        );
...
const openDeleteDomain = (domain: DomainResponse) => {
    selectedDomain.value = domain;
    showDeleteDomainModal.value = true;
};
const openDeleteSubdomain = (sub: SubdomainResponse) => {
    selectedSubdomain.value = sub;
    showDeleteSubdomainModal.value = true;
};
...
const openDeleteDns = (record: DnsRecordResponse) => {
    selectedDnsRecord.value = record;
    showDeleteDnsModal.value = true;
};
const openDeleteRedirect = (redirect: any) => {
    selectedRedirect.value = redirect;
    showDeleteRedirectModal.value = true;
};
const openDeleteAlias = (alias: any) => {
...
<template>
    <MainLayout>
        <div class="max-w-7xl mx-auto space-y-6 animate-in">
            ...
            >
                <div>
                    ...
                    <p class="text-muted-foreground mt-1 text-sm">
                        Manage domains, subdomains, SSL, redirects, and DNS.
                    </p>
                </div>
                ...
                <div class="flex gap-2">
                    ...
                    <button
                        v-if="activeTab === 'subdomains' && selectedDomainId"
                        @click="showAddSubdomainModal = true"
                        class="btn-primary bg-indigo-600 hover:bg-indigo-700 shadow-indigo-600/20"
                    >
                        <Plus class="w-4 h-4" /> Add Subdomain
                    </button>
                    <button
                        v-if="activeTab === 'ssl' && selectedDomainId"
                        @click="showRequestSslModal = true"
                        class="btn-primary bg-emerald-600 hover:bg-emerald-700 shadow-emerald-600/20"
                    >
                        <Lock class="w-4 h-4" /> Request SSL
                    </button>
                    <button
                        v-if="activeTab === 'dns' && selectedDomainId"
                        @click="showAddDnsModal = true"
                        class="btn-primary bg-blue-600 hover:bg-blue-700 shadow-blue-600/20"
                    >
                        <Plus class="w-4 h-4" /> Add Record
                    </button>
                    <button
                        v-if="activeTab === 'redirects' && selectedDomainId"
                        @click="showAddRedirectModal = true"
                        class="btn-primary bg-amber-600 hover:bg-amber-700 shadow-amber-600/20"
                    >
                        <CornerUpRight class="w-4 h-4" /> Add Redirect
                    </button>
                    <button
                        v-if="activeTab === 'aliases' && selectedDomainId"
                        @click="showAddAliasModal = true"
                        class="btn-primary bg-purple-600 hover:bg-purple-700 shadow-purple-600/20"
                    >
            ...
            <!-- Domain Selector -->
            <div
                v-if="activeTab !== 'domains' && domains.length > 0"
                class="bg-card border border-border p-4 rounded-xl flex items-center gap-4"
            >
                <label
                    class="text-sm font-medium text-foreground whitespace-nowrap"
                    >Managing Domain:</label
                >
                <div class="relative flex-1 max-w-md">
                    <select
                        v-model="selectedDomainId"
                        class="w-full bg-muted border border-border rounded-lg px-4 py-2.5 text-sm focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none appearance-none"
                    >
                        <option
                    ...
                    <ChevronDown
                        class="w-4 h-4 text-muted-foreground absolute right-4 top-1/2 -translate-y-1/2 pointer-events-none"
                    />
                </div>
            </div>
            ...
            >
                ...
                >
                    ...
                    <button @click="fetchDomains" class="mt-4 btn-secondary">
                        Retry
                    </button>
                </div>
                ...
                <div v-else>
                            ...
                            <tbody class="divide-y divide-border">
                                ...
                                >
                                    ...
                                    <td class="px-6 py-4 text-right">
                                        <button
                                            @click="openDeleteDomain(domain)"
                                            class="icon-btn-danger group-hover:opacity-100 opacity-0"
                                        >
                    ...
                    <div v-else-if="activeTab === 'subdomains'">
                        <div v-if="subdomains.length === 0" class="empty-state">
                            ...
                            <h3>No subdomains</h3>
                            <p>
                                Create subdomains like blog.{{
                                    selectedDomainObj?.domain_name
                                }}
                            </p>
                        </div>
                        <table v-else class="w-full text-left">
                        ...
                        <table v-else class="w-full text-left">
                            ...
                            <tbody class="divide-y divide-border">
                                ...
                                >
                                    ...
                                    <td
                                        class="px-6 py-4 text-sm font-mono truncate max-w-[200px]"
                                    >
                                        {{ record.value }}
                                    </td>
                                    <td
                                    ...
                                    <td
                                        class="px-6 py-4 text-right flex justify-end gap-1"
                                    >
                                        <button
                                        ...
                                        <button
                                            @click="openDeleteDns(record)"
                                            class="icon-btn-danger opacity-0 group-hover:opacity-100"
                                        >
                                            <Trash2 class="w-4 h-4" />
                                        </button>
                                    </td>
        ...
        <div
            ...
            class="modal-overlay"
            @click.self="showAddDomainModal = false"
        >
            <div class="modal-content animate-in">
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        <label class="form-label"
                            >Document Root (Optional)</label
                        >
                        <input
                            v-model="newDocumentRoot"
                            type="text"
                ...
                <div class="modal-footer">
                    ...
                    <button @click="createDomain" class="btn-primary">
                        Add Domain
                    </button>
                </div>
            </div>
        ...
        <div
            ...
            class="modal-overlay"
            @click.self="showAddAliasModal = false"
        >
            <div class="modal-content animate-in">
                <div class="modal-header">
                    <h3>Add Parked Domain (Alias)</h3>
                    <button @click="showAddAliasModal = false">
                        <X class="w-5 h-5 text-muted-foreground" />
                    </button>
                </div>
                <div class="space-y-4">
                ...
                <div class="modal-footer">
                    ...
                    <button @click="createAlias" class="btn-primary">
                        Add Alias
                    </button>
                </div>
            </div>
        ...
        <!-- Add Redirect Modal -->
        <div
            v-if="showAddRedirectModal"
            class="modal-overlay"
            @click.self="showAddRedirectModal = false"
        >
            <div class="modal-content animate-in">
                <div class="modal-header">
                    <h3>Add Redirect</h3>
                    <button @click="showAddRedirectModal = false">
                        <X class="w-5 h-5 text-muted-foreground" />
                    </button>
                </div>
                <div class="space-y-4">
                ...
                <div class="modal-footer">
                    <button
                        @click="showAddRedirectModal = false"
                        class="btn-ghost"
                    >
                        Cancel
                    </button>
                    <button @click="createRedirect" class="btn-primary">
                        Add Redirect
                    </button>
                </div>
            </div>
        ...
        <div
            v-if="showDeleteDomainModal"
            class="modal-overlay"
            @click.self="showDeleteDomainModal = false"
        >
            <div class="modal-content animate-in">
                <h3>Delete Domain?</h3>
                <div class="modal-footer">
                    <button
                        @click="showDeleteDomainModal = false"
                        class="btn-ghost"
                    >
                        Cancel
                    </button>
                    <button @click="deleteDomain" class="btn-destroy">
                        Delete
                    </button>
                </div>
            </div>
        ...
        <!-- Add Subdomain Modal -->
        <div
            v-if="showAddSubdomainModal"
            class="modal-overlay"
            @click.self="showAddSubdomainModal = false"
        >
            <div class="modal-content animate-in">
                <div class="modal-header">
                    <h3>Add Subdomain</h3>
                    <button @click="showAddSubdomainModal = false">
                        <X class="w-5 h-5 text-muted-foreground" />
                    </button>
                </div>
                <div class="space-y-4">
                    <div>
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <span class="text-sm text-muted-foreground"
                                >.{{ selectedDomainObj?.domain_name }}</span
                            >
                        </div>
                    </div>
                    <div>
                ...
                <div class="modal-footer">
                    <button
                        @click="showAddSubdomainModal = false"
                        class="btn-ghost"
                    >
                        Cancel
                    </button>
                    <button @click="createSubdomain" class="btn-primary">
                        Create
                    </button>
                </div>
            </div>
        ...
        <!-- Add DNS Modal -->
        <div
            v-if="showAddDnsModal"
            class="modal-overlay"
            @click.self="showAddDnsModal = false"
        >
            <div class="modal-content animate-in">
                <div class="modal-header">
                    <h3>Add DNS Record</h3>
                    <button @click="showAddDnsModal = false">
                        <X class="w-5 h-5 text-muted-foreground" />
                    </button>
                </div>
                <div class="space-y-4">
                ...
                <div class="modal-footer">
                    <button @click="showAddDnsModal = false" class="btn-ghost">
                        Cancel
                    </button>
                    <button @click="createDnsRecord" class="btn-primary">
                        Add Record
                    </button>
                </div>
            </div>
        ...
        <!-- Delete Subdomain -->
        <div
            v-if="showDeleteSubdomainModal"
            class="modal-overlay"
            @click.self="showDeleteSubdomainModal = false"
        >
            <div class="modal-content animate-in">
                <h3>Delete Subdomain?</h3>
                <p class="text-sm text-muted-foreground mt-2">
                    Are you sure you want to delete
                    {{ selectedSubdomain?.subdomain_name }}?
                </p>
                <div class="modal-footer">
                    <button
                        @click="showDeleteSubdomainModal = false"
                        class="btn-ghost"
                    >
                        Cancel
                    </button>
                    <button @click="deleteSubdomain" class="btn-destroy">
                        Delete
                    </button>
                </div>
            </div>
        ...
        <!-- Delete DNS -->
        <div
            v-if="showDeleteDnsModal"
            class="modal-overlay"
            @click.self="showDeleteDnsModal = false"
        >
            <div class="modal-content animate-in">
                <h3>Delete DNS Record?</h3>
                <p class="text-sm text-muted-foreground mt-2">
                    Delete {{ selectedDnsRecord?.name }} ({{
                        selectedDnsRecord?.record_type
                    }})?
                </p>
                <div class="modal-footer">
                    <button
                        @click="showDeleteDnsModal = false"
                        class="btn-ghost"
                    >
                        Cancel
                    </button>
                    <button @click="deleteDnsRecord" class="btn-destroy">
                        Delete
                    </button>
                </div>
            </div>
        ...
        <!-- Edit DNS -->
        <div
            v-if="showEditDnsModal"
            class="modal-overlay"
            @click.self="showEditDnsModal = false"
        >
            <div class="modal-content animate-in">
                <div class="modal-header">
                    <h3>Edit DNS Record</h3>
                    <button @click="showEditDnsModal = false">
                        <X class="w-5 h-5 text-muted-foreground" />
                    </button>
                </div>
                <div class="space-y-4">
                ...
                <div class="modal-footer">
                    <button @click="showEditDnsModal = false" class="btn-ghost">
                        Cancel
                    </button>
                    <button @click="updateDnsRecord" class="btn-primary">
                        Save Changes
                    </button>
                </div>
            </div>
        ...
        <!-- Delete Redirect -->
        <div
            v-if="showDeleteRedirectModal"
            class="modal-overlay"
            @click.self="showDeleteRedirectModal = false"
        >
            <div class="modal-content animate-in">
                <h3>Delete Redirect?</h3>
                <div class="modal-footer">
                    <button
                        @click="showDeleteRedirectModal = false"
                        class="btn-ghost"
                    >
                        Cancel
                    </button>
                    <button @click="deleteRedirect" class="btn-destroy">
                        Delete
                    </button>
                </div>
            </div>
        ...
        <!-- Delete Alias -->
        <div
            v-if="showDeleteAliasModal"
            class="modal-overlay"
            @click.self="showDeleteAliasModal = false"
        >
            <div class="modal-content animate-in">
                <h3>Delete Alias?</h3>
                <div class="modal-footer">
                    <button
                        @click="showDeleteAliasModal = false"
                        class="btn-ghost"
                    >
                        Cancel
                    </button>
                    <button @click="deleteAlias" class="btn-destroy">
                        Delete
                    </button>
                </div>
            </div>
        ...
        <!-- Request SSL -->
        <div
            v-if="showRequestSslModal"
            class="modal-overlay"
            @click.self="showRequestSslModal = false"
        >
            <div class="modal-content animate-in">
                <div class="modal-header">
                    <h3>Request SSL Certificate</h3>
                    <button @click="showRequestSslModal = false">
                        <X class="w-5 h-5 text-muted-foreground" />
                    </button>
                </div>
                <p class="text-sm text-foreground mb-4">
                    Install specific SSL for
                    <strong>{{ selectedDomainObj?.domain_name }}</strong
                    >?
                </p>
                <div class="bg-muted p-4 rounded-lg mb-4">
                    <p class="text-xs text-muted-foreground">
                        Provider: <strong>Let's Encrypt (Free)</strong>
                    </p>
                    <p class="text-xs text-muted-foreground mt-1">
                        Validity: <strong>90 Days (Auto-renew)</strong>
                    </p>
                </div>
                <div class="modal-footer">
                    <button
                        @click="showRequestSslModal = false"
                        class="btn-ghost"
                    >
                        Cancel
                    </button>
                    <button
                        @click="
                            showRequestSslModal = false;
                            showSuccess('SSL Requested (Mock)');
                        "
...
.btn-primary {
    @apply flex items-center gap-2 px-5 py-2.5 bg-primary hover:bg-primary/90 text-primary-foreground rounded-xl font-medium transition-all shadow-lg shadow-primary/20 active:scale-[0.98] text-sm;
}
.btn-secondary {
    @apply px-5 py-2.5 bg-muted hover:bg-muted/80 text-foreground rounded-xl font-medium transition-all text-sm;
}
.btn-ghost {
    @apply px-4 py-2 text-sm font-medium text-muted-foreground hover:text-foreground transition-colors;
}
.btn-destroy {
    @apply px-4 py-2 text-sm font-medium bg-destructive text-destructive-foreground rounded-lg hover:bg-destructive/90 transition-colors;
}
.icon-btn {
    @apply p-2 text-muted-foreground hover:text-primary hover:bg-primary/10 rounded-lg transition-all;
}
.icon-btn-danger {
    @apply p-2 text-muted-foreground hover:text-destructive hover:bg-destructive/10 rounded-lg transition-all;
}
...
.modal-content {
    @apply bg-card border border-border rounded-xl shadow-xl w-full max-w-md p-6;
}
.modal-header {
...
.form-input {
    @apply w-full bg-muted border border-border rounded-lg px-4 py-3 text-sm focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none;
}
...
.table-header {
    @apply bg-muted/50 border-b border-border text-[10px] font-semibold text-muted-foreground uppercase tracking-wider;
}
.badge {
    @apply inline-flex items-center gap-1.5 px-2 py-1 rounded-lg text-[10px] font-semibold uppercase;
}
.badge-success {
