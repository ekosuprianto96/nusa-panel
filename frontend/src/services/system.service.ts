...
export const systemService = {
  ...
  dnsLookup: (domain: string, record_type: string) =>
    api.post<ApiResponse<string>>('/system/dns/lookup', { domain, record_type }),

  traceRoute: (domain: string) =>
    api.post<ApiResponse<string[]>>('/system/dns/traceroute', { domain }),

  // Resource Usage
  getResourceUsage: () =>
    api.get<ApiResponse<ResourceUsage>>('/system/resources')
}
